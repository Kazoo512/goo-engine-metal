...
#define ADD_SHGROUP_CALL(shgrp, ob_ref, geom, oedata) \
  do { \
    if (oedata) { \
      DRW_shgroup_call_with_callback(shgrp, geom, ob_ref, oedata); \
    } \
    else { \
      DRW_shgroup_call(shgrp, geom, ob_ref); \
    } \
  } while (0)
  ...
  if (ELEM(ob->type, OB_MESH, OB_SURF)) {
    ...
    if ((ob->dt >= OB_SOLID) || DRW_state_is_scene_render()) {
      if (use_sculpt_pbvh) {
        ...
        DRW_shgroup_call_sculpt_with_materials(shgrps_array, gpumat_array, materials_len, ob_ref);
      ...
      /* Motion Blur Vectors. */
      EEVEE_motion_blur_cache_populate(sldata, vedata, ob_ref);
    }

    /* Volumetrics */
    if (use_volume_material) {
      EEVEE_volumes_cache_object_add(sldata, vedata, scene, ob_ref);
    }
  }
}
    ...
    if (ob != draw_ctx->object_edit) {
      LISTBASE_FOREACH (ModifierData *, md, &ob->modifiers) {
        ...
        if (matcache.depth_grp) {
          *matcache.depth_grp_p = blender::draw::DRW_shgroup_hair_create_sub(
              ob_ref, psys, md, matcache.depth_grp, nullptr);
        }
        if (matcache.shading_grp) {
          *matcache.shading_grp_p = blender::draw::DRW_shgroup_hair_create_sub(
              ob_ref, psys, md, matcache.shading_grp, matcache.shading_gpumat);
        }
        if (matcache.shadow_grp) {
          *matcache.shadow_grp_p = blender::draw::DRW_shgroup_hair_create_sub(
              ob_ref, psys, md, matcache.shadow_grp, nullptr);
          *cast_shadow = true;
        }

        EEVEE_motion_blur_hair_cache_populate(sldata, vedata, ob_ref, psys, md);
      }
    }
  }
}
                                        ...
                                        bool *cast_shadow)
{
  ...
  if (matcache.depth_grp) {
    *matcache.depth_grp_p = DRW_shgroup_curves_create_sub(ob_ref, matcache.depth_grp, nullptr);
  }
  if (matcache.shading_grp) {
    *matcache.shading_grp_p = DRW_shgroup_curves_create_sub(
        ob_ref, matcache.shading_grp, matcache.shading_gpumat);
  }
  if (matcache.shadow_grp) {
    *matcache.shadow_grp_p = DRW_shgroup_curves_create_sub(ob_ref, matcache.shadow_grp, nullptr);
    *cast_shadow = true;
  }

  EEVEE_motion_blur_curves_cache_populate(sldata, vedata, ob_ref);
}
