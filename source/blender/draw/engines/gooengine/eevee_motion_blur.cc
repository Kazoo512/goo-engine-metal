...
int EEVEE_motion_blur_init(EEVEE_ViewLayerData * /*sldata*/, EEVEE_Data *vedata)
{
  ...
  if ((effects->motion_blur_max > 0) && (scene->r.mode & R_MBLUR)) {
    ...
    const float *fs_size = GOO_viewport_size_get();
    const int tx_size[2] = {
...
void EEVEE_motion_blur_cache_init(EEVEE_ViewLayerData * /*sldata*/, EEVEE_Data *vedata)
{
  ...
  if ((effects->enabled_effects & EFFECT_MOTION_BLUR) != 0) {
    ...
    DRWShadingGroup *grp;
    {
      ...
      DRW_shgroup_uniform_vec2(grp, "viewportSize", GOO_viewport_size_get(), 1);
                                           ...
                                           ModifierData *md)
{
  ...
  EEVEE_ObjectMotionData *mb_data = EEVEE_motion_blur_object_data_get(
      &effects->motion_blur, ob_ref, true);
  ...
  if (mb_data) {
    ...
    if (mb_step == MB_CURR) {
      ...
      grp = blender::draw::DRW_shgroup_hair_create_sub(ob_ref, psys, md, effects->motion_blur.hair_grp, nullptr);
                                             ...
                                             const blender::draw::ObjectRef &ob_ref)
{
  ...
  EEVEE_ObjectMotionData *mb_data = EEVEE_motion_blur_object_data_get(
      &effects->motion_blur, ob_ref, false);
  if (mb_data == nullptr) {
    return;
  }
  ...
  if (mb_step == MB_CURR) {
    ...
    DRWShadingGroup *grp = DRW_shgroup_curves_create_sub(
        ob_ref, effects->motion_blur.hair_grp, nullptr);
    DRW_shgroup_uniform_mat4(grp, "prevModelMatrix", mb_data->obmat[MB_PREV]);
    DRW_shgroup_uniform_mat4(grp, "currModelMatrix", mb_data->obmat[MB_CURR]);
    DRW_shgroup_uniform_mat4(grp, "nextModelMatrix", mb_data->obmat[MB_NEXT]);
    DRW_shgroup_uniform_texture(grp, "prvBuffer", tex_prev);
                                      ...
                                      const blender::draw::ObjectRef &ob_ref)
{
  ...
  if (mb_data) {
    ...
    if (mb_step == MB_CURR) {
      ...
      DRW_shgroup_call(grp, batch, ob_ref);
    }
    else if (is_deform) {
      /* Store vertex position buffer. */
