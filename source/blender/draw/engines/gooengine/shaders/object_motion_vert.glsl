...
void main()
{
...
#ifdef HAIR
  bool is_persp = (ProjectionMatrixGoo[3][3] == 0.0);
  float time, thick_time, thickness;
  vec3 tan, binor;
  vec3 wpos;

  hair_get_pos_tan_binor_time(is_persp,
                              ModelMatrixInverseGoo,
                              ViewMatrixInverseGoo[3].xyz,
                              ViewMatrixInverseGoo[2].xyz,
                              wpos,
                              tan,
                              binor,
                              time,
   ...
   * Note that currModelMatrix needs to also be equal to ModelMatrixGoo for the samples to match. */
#ifndef HAIR
  gl_Position = ProjectionMatrixGoo * (ViewMatrixGoo * vec4(currWorldPos, 1.0));
#else
  gl_Position = ProjectionMatrixGoo * (ViewMatrixGoo * vec4(wpos, 1.0));
#endif
}
