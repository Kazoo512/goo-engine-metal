...
{
  vec4 v = (ProjectionMatrixGoo[3][3] == 0.0) ? vec4(viewvec, 1.0) : vec4(0.0, 0.0, 1.0, 1.0);
  vec4 co_homogeneous = (ProjectionMatrixInverseGoo * v);

  vec4 co = vec4(co_homogeneous.xyz / co_homogeneous.w, 0.0);
  return (ViewMatrixInverseGoo * co).xyz;
}
