...
void main()
{
  ...
  vec3 P = transform_point(ViewMatrixInverseGoo, vP);
  vec3 vV = viewCameraVec(vP);
  vec3 V = cameraVec(P);
  vec3 vN = normal_decode(textureLod(normalBuffer, uvs, 0.0).rg, vV);
  vec3 N = transform_direction(ViewMatrixInverseGoo, vN);
  ...
  for (int i = 0; i < MAX_PLANAR && i < prbNumPlanar; i++) {
    ...
    if (fade > 0.5) {
      ...
      vP = transform_point(ViewMatrixGoo, P_plane);
  ...
  if (data.is_planar) {
    vec3 view_plane_normal = transform_direction(ViewMatrixGoo, planars_data[planar_id].pl_normal);
