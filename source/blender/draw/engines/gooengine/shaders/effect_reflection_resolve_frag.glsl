...
vec4 ssr_get_scene_color_and_mask(vec3 hit_vP, int planar_index, float mip)
{
  ...
  if (planar_index != -1) {
    ...
    vec3 hit_P = transform_point(ViewMatrixInverseGoo, hit_vP);
                               ...
                               inout vec4 ssr_accum)
{
  ...
  float homcoord = ProjectionMatrixGoo[2][3] * hit_vP.z + ProjectionMatrixGoo[3][3];
  cone_footprint *= max(ProjectionMatrixGoo[0][0], ProjectionMatrixGoo[1][1]) / homcoord;
                      ...
                      inout ClosureOutputGlossy cl_out)
{
  ...
  if (roughness < ssrMaxRoughness + 0.2) {
    ...
    /* Using view space */
    vec3 vV = transform_direction(ViewMatrixGoo, cl_common.V);
    vec3 vP = transform_point(ViewMatrixGoo, cl_common.P);
    vec3 vN = transform_direction(ViewMatrixGoo, cl_in.N);
...
void main()
{
  ...
  worldPosition = transform_point(ViewMatrixInverseGoo, viewPosition);

  vec2 normal_encoded = texelFetch(normalBuffer, texel, 0).rg;
  viewNormal = normal_decode(normal_encoded, viewCameraVec(viewPosition));
  worldNormal = transform_direction(ViewMatrixInverseGoo, viewNormal);
