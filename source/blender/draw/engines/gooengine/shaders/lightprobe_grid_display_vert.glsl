...
void main()
{
  ...
  vec3 screen_pos = ViewMatrixInverseGoo[0].xyz * quadCoord.x +
                    ViewMatrixInverseGoo[1].xyz * quadCoord.y;
  ws_cell_location += screen_pos * sphere_size;

  gl_Position = ProjectionMatrixGoo * (ViewMatrixGoo * vec4(ws_cell_location, 1.0));
