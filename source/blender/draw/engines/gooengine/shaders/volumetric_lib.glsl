...
vec3 participating_media_extinction(vec3 wpos, sampler3D volume_extinction)
{
  ...
  vec3 ndc = project_point(ProjectionMatrixGoo, transform_point(ViewMatrixGoo, wpos));
...
vec3 light_volume_shadow(LightData ld, vec3 ray_wpos, vec4 l_vector, sampler3D volume_extinction)
{
  ...
  if (ld.l_type == SUN) {
    ...
    vec3 ndcP = project_point(ProjectionMatrixGoo, transform_point(ViewMatrixGoo, ray_wpos));
    vec3 ndcL = project_point(ProjectionMatrixGoo,
                              transform_point(ViewMatrixGoo, ray_wpos + l_vector.xyz)) -
                ndcP;

    vec3 frustum_isect = ndcP + ndcL * line_unit_box_intersect_dist_safe(ndcP, ndcL);

    vec4 L_hom = ViewMatrixInverseGoo * (ProjectionMatrixInverseGoo * vec4(frustum_isect, 1.0));
