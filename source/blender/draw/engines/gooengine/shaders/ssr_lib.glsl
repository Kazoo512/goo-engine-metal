...
vec4 screen_space_refraction(vec3 vP, vec3 N, vec3 V, float ior, float roughnessSquared, vec4 rand)
{
  ...
  R = transform_direction(ViewMatrixGoo, R);
  ...
  if (hit && (F_eta(ior, dot(H, V)) < 1.0)) {
    ...
    float homcoord = ProjectionMatrixGoo[2][3] * hit_pos.z + ProjectionMatrixGoo[3][3];
    /* UV space footprint */
    cone_footprint = BTDF_BIAS * 0.5 * max(ProjectionMatrixGoo[0][0], ProjectionMatrixGoo[1][1]) *
                     cone_footprint / homcoord;

    vec2 hit_uvs = project_point(ProjectionMatrixGoo, hit_pos).xy * 0.5 + 0.5;
