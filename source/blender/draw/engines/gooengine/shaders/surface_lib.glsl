...
#    if defined(GPU_OPENGL)
  ...
  vec3 pos0 = (ProjectionMatrixInverseGoo * gpu_position_at_vertex(0)).xyz;
  vec3 pos1 = (ProjectionMatrixInverseGoo * gpu_position_at_vertex(1)).xyz;
  vec3 pos2 = (ProjectionMatrixInverseGoo * gpu_position_at_vertex(2)).xyz;
...
#  if defined(WORLD_BACKGROUND) || defined(PROBE_CAPTURE)
  surf.P = transform_direction(ViewMatrixInverseGoo, -viewCameraVec(viewPosition));
...
vec3 coordinate_camera(vec3 P)
{
  ...
  vP = transform_direction(ViewMatrixGoo, P);
#else
  vP = transform_point(ViewMatrixGoo, P);
#endif
  vP.z = -vP.z;
  return vP;
}
...
vec3 coordinate_screen(vec3 P)
{
  ...
  window.xy = project_point(ProjectionMatrixGoo, viewPosition).xy * 0.5 + 0.5;
  window.xy = window.xy * cameraUvScaleBias.xy + cameraUvScaleBias.zw;

#elif defined(common_UNIFORMS_LIB) /* MESH */
  window.xy = project_point(ProjectionMatrixGoo, transform_point(ViewMatrixGoo, P)).xy * 0.5 + 0.5;
  window.xy = window.xy * cameraUvScaleBias.xy + cameraUvScaleBias.zw;
#endif
  return window;
}
