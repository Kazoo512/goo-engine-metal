...
bool EEVEE_render_init(EEVEE_Data *ved, RenderEngine *engine, Depsgraph *depsgraph)
{
  ...
  const float *size_orig = GOO_viewport_size_get();
  float size_final[2];
...
void EEVEE_render_cache(void *vedata, blender::draw::ObjectRef &ob_ref, RenderEngine *engine, Depsgraph *depsgraph)
{
  EEVEE_ViewLayerData *sldata = EEVEE_view_layer_data_ensure();
  ...
  if (ob_visibility & OB_VISIBLE_PARTICLES) {
    EEVEE_particle_hair_cache_populate(
        static_cast<EEVEE_Data *>(vedata), sldata, ob_ref, &cast_shadow);
    if (do_cryptomatte) {
      EEVEE_cryptomatte_particle_hair_cache_populate(data, sldata, ob_ref);
    }
  }

  if (ob_visibility & OB_VISIBLE_SELF) {
    if (ob->type == OB_MESH) {
      EEVEE_materials_cache_populate(static_cast<EEVEE_Data *>(vedata), sldata, ob_ref, &cast_shadow);
      if (do_cryptomatte) {
        EEVEE_cryptomatte_cache_populate(data, sldata, ob_ref);
      }
    }
      ...
      EEVEE_object_curves_cache_populate(
          static_cast<EEVEE_Data *>(vedata), sldata, ob_ref, &cast_shadow);
      if (do_cryptomatte) {
        EEVEE_cryptomatte_object_curves_cache_populate(data, sldata, ob_ref);
      }
    }
    else if (ob->type == OB_VOLUME) {
      Scene *scene = DEG_get_evaluated_scene(depsgraph);
      EEVEE_volumes_cache_object_add(sldata, static_cast<EEVEE_Data *>(vedata), scene, ob_ref);
    }
