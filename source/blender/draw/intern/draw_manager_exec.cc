...
{
  /* Mask locked state. */
  state = (~drw_get().state_lock & state) | (drw_get().state_lock & drw_get().state);

  if (drw_get().state == state) {
    return;
  }
  ...
  if (state & DRW_STATE_CLIP_PLANES) {
    GPU_clip_distances(drw_get().view_active->clip_planes_len);
  }
  else {
...
static void drw_state_validate()
{
  ...
  if (drw_get().state & DRW_STATE_WRITE_STENCIL_ENABLED) {
    BLI_assert(drw_get().state & DRW_STATE_STENCIL_TEST_ENABLED);
  }
  /* Cannot write to depth buffer without depth test. */
  if (drw_get().state & DRW_STATE_WRITE_DEPTH) {
    BLI_assert(drw_get().state & DRW_STATE_DEPTH_TEST_ENABLED);
  }
}

void DRW_state_lock(DRWState state)
{
  drw_get().state_lock = state;

  /* We must get the current state to avoid overriding it. */
  /* Not complete, but that just what we need for now. */
  if (state & DRW_STATE_WRITE_DEPTH) {
    SET_FLAG_FROM_TEST(drw_get().state, GPU_depth_mask_get(), DRW_STATE_WRITE_DEPTH);
  }
  if (state & DRW_STATE_DEPTH_TEST_ENABLED) {
    drw_get().state &= ~DRW_STATE_DEPTH_TEST_ENABLED;

    switch (GPU_depth_test_get()) {
      case GPU_DEPTH_ALWAYS:
        drw_get().state |= DRW_STATE_DEPTH_ALWAYS;
        break;
      case GPU_DEPTH_LESS:
        drw_get().state |= DRW_STATE_DEPTH_LESS;
        break;
      case GPU_DEPTH_LESS_EQUAL:
        drw_get().state |= DRW_STATE_DEPTH_LESS_EQUAL;
        break;
      case GPU_DEPTH_EQUAL:
        drw_get().state |= DRW_STATE_DEPTH_EQUAL;
        break;
      case GPU_DEPTH_GREATER:
        drw_get().state |= DRW_STATE_DEPTH_GREATER;
        break;
      case GPU_DEPTH_GREATER_EQUAL:
        drw_get().state |= DRW_STATE_DEPTH_GREATER_EQUAL;
        break;
      default:
        break;
    }
...
static bool draw_call_is_culled(const DRWResourceHandle *handle, DRWView *view)
{
  DRWCullingState *culling = static_cast<DRWCullingState *>(
      DRW_memblock_elem_from_handle(drw_get().data->cullstates, handle));
  return (culling->mask & view->culling_mask) != 0;
}

void DRW_view_set_active(const DRWView *view)
{
  drw_get().view_active = (view != nullptr) ? ((DRWView *)view) : drw_get().view_default;
}

const DRWView *DRW_view_get_active()
{
  return drw_get().view_active;
}
...
static void draw_compute_culling(DRWView *view)
{
  ...
  BLI_memblock_iternew(drw_get().data->cullstates, &iter);
...
BLI_INLINE void draw_indirect_call(DRWShadingGroup *shgroup, DRWCommandsState *state)
{
  ...
  if (state->baseinst_loc == -1) {
    /* bind vertex array */
    if (drw_get().batch != state->batch) {
      GPU_draw_list_submit(drw_get().draw_list);
      draw_geometry_bind(shgroup, state->batch);
    }
    GPU_draw_list_append(drw_get().draw_list, state->batch, state->base_inst, state->inst_count);
  }
    ...
    for (int i = 0; i < unichunk->uniform_used; i++, uni--) {
      ...
      switch (uni->type) {
        ...
        case DRW_UNIFORM_BLOCK_OBMATS:
          ...
          GPU_uniformbuf_bind(drw_get().data->matrices_ubo[0], uni->location);
          break;
        case DRW_UNIFORM_BLOCK_OBINFOS:
          state->obinfos_loc = uni->location;
          GPU_uniformbuf_bind(drw_get().data->obinfos_ubo[0], uni->location);
          break;
        case DRW_UNIFORM_BLOCK_OBATTRS:
          state->obattrs_loc = uni->location;
          state->obattrs_ubo = DRW_uniform_attrs_pool_find_ubo(drw_get().data->obattrs_ubo_pool,
                                                               uni->uniform_attrs);
...
static void draw_call_resource_bind(DRWCommandsState *state, const DRWResourceHandle *handle)
{
  ...
  if (neg_scale != state->neg_scale) {
    ...
    GPU_front_facing(neg_scale != drw_get().view_active->is_inverted);
  }

  int chunk = DRW_handle_chunk_get(handle);
  if (state->resource_chunk != chunk) {
    if (state->chunkid_loc != -1) {
      GPU_shader_uniform_int_ex(drw_get().shader, state->chunkid_loc, 1, 1, &chunk);
    }
    if (state->obmats_loc != -1) {
      GPU_uniformbuf_unbind(drw_get().data->matrices_ubo[state->resource_chunk]);
      GPU_uniformbuf_bind(drw_get().data->matrices_ubo[chunk], state->obmats_loc);
    }
    if (state->obinfos_loc != -1) {
      GPU_uniformbuf_unbind(drw_get().data->obinfos_ubo[state->resource_chunk]);
      GPU_uniformbuf_bind(drw_get().data->obinfos_ubo[chunk], state->obinfos_loc);
    }
  ...
  if (state->resourceid_loc != -1) {
    ...
    if (state->resource_id != id) {
      GPU_shader_uniform_int_ex(drw_get().shader, state->resourceid_loc, 1, 1, &id);
...
static void draw_call_batching_finish(DRWShadingGroup *shgroup, DRWCommandsState *state)
{
  ...
  /* Reset state */
  if (state->neg_scale) {
    GPU_front_facing(drw_get().view_active->is_inverted);
  }
  if (state->obmats_loc != -1) {
    GPU_uniformbuf_unbind(drw_get().data->matrices_ubo[state->resource_chunk]);
  }
  ...
  if (state->vlattrs_loc != -1) {
    GPU_uniformbuf_unbind(drw_get().data->vlattrs_ubo);
  }
}
...
static void draw_shgroup(DRWShadingGroup *shgroup, DRWState pass_state)
{
  ...
  const bool shader_changed = (drw_get().shader != shgroup->shader);
  ...
  if (shader_changed) {
    ...
    drw_get().shader = shgroup->shader;
    drw_get().batch = nullptr;
  }

  draw_update_uniforms(shgroup, &state);
    ...
    while ((cmd = draw_command_iter_step(&iter, &cmd_type))) {
      ...
      switch (cmd_type) {
        ...
        case DRW_CMD_DRAW_INSTANCE:
          if (draw_call_is_culled(&cmd->instance.handle, drw_get().view_active)) {
            continue;
          }
          break;
        default:
...
static void drw_update_view()
{
  ...
  GPU_uniformbuf_update(G_draw.view_ubo, &drw_get().view_active->storage);
  GPU_uniformbuf_update(G_draw.clipping_ubo, &drw_get().view_active->clip_planes);

  draw_compute_culling(drw_get().view_active);
}
                             ...
                             DRWShadingGroup *end_group)
{
  ...
  drw_get().shader = nullptr;

  BLI_assert_msg(drw_get().buffer_finish_called,
                 "DRW_render_instance_buffer_finish had not been called before drawing");

  if (drw_get().view_previous != drw_get().view_active || drw_get().view_active->is_dirty) {
    drw_update_view();
    drw_get().view_active->is_dirty = false;
    drw_get().view_previous = drw_get().view_active;
  }
  ...
  drw_state_set(drw_get().state | DRW_STATE_WRITE_DEPTH | DRW_STATE_WRITE_COLOR);

  drw_state_set(pass->state);
  drw_state_validate();

  if (drw_get().view_active->is_inverted) {
  ...
  if ((drw_get().state & DRW_STATE_BLEND_CUSTOM) != 0 &&
      GPU_type_matches(GPU_DEVICE_ATI, GPU_OS_ANY, GPU_DRIVER_OFFICIAL))
  {
    drw_state_set(drw_get().state & ~DRW_STATE_BLEND_CUSTOM);
  }
  ...
  if ((drw_get().state & DRW_STATE_RASTERIZER_ENABLED) == 0) {
    drw_state_set((drw_get().state & ~DRW_STATE_RASTERIZER_ENABLED) | DRW_STATE_DEFAULT);
  }
