...
// #define IS_DEBUG_MOUSE_FRAGMENT (ivec2(gl_FragCoord) == drw_view_goo.mouse_pixel)
#define IS_FIRST_INVOCATION (gl_GlobalInvocationID == uvec3(0))

#define cameraForward ViewMatrixInverseGoo[2].xyz
#define cameraPos ViewMatrixInverseGoo[3].xyz
vec3 cameraVec(vec3 P)
{
  return ((ProjectionMatrixGoo[3][3] == 0.0) ? normalize(cameraPos - P) : cameraForward);
}
#define viewCameraVec(vP) ((ProjectionMatrixGoo[3][3] == 0.0) ? normalize(-vP) : vec3(0.0, 0.0, 1.0))

#ifdef COMMON_GLOBALS_LIB
/* TODO move to overlay engine. */
float mul_project_m4_v3_zfac(vec3 co)
{
  vec3 vP = (ViewMatrixGoo * vec4(co, 1.0)).xyz;
  return pixelFac * ((ProjectionMatrixGoo[0][3] * vP.x) + (ProjectionMatrixGoo[1][3] * vP.y) +
                     (ProjectionMatrixGoo[2][3] * vP.z) + ProjectionMatrixGoo[3][3]);
}
#endif
...
#    define ModelMatrixGoo (drw_matrices[resource_id].model)
#    define ModelMatrixInverseGoo (drw_matrices[resource_id].model_inverse)
#  endif /* USE_GPU_SHADER_CREATE_INFO */
...
uniform mat4 ModelMatrixGoo;
uniform mat4 ModelMatrixInverseGoo;
#  endif /* !USE_GPU_SHADER_CREATE_INFO */
...
/**
 * Some clarification:
 * Usually Normal matrix is transpose(inverse(ViewMatrixGoo * ModelMatrixGoo))
 *
 ...
 * transpose(ViewMatrixInverseGoo) * transpose(ModelMatrixInverseGoo)
 *
 * Knowing that the view matrix is orthogonal, the transpose is also the inverse.
 * NOTE: This is only valid because we are only using the mat3 of the ViewMatrixInverseGoo.
 * ViewMatrixGoo * transpose(ModelMatrixInverseGoo)
 */
#define NormalMatrix transpose(mat3(ModelMatrixInverseGoo))
#define NormalMatrixInverse transpose(mat3(ModelMatrixGoo))

#define normal_object_to_view(n) (mat3(ViewMatrixGoo) * (NormalMatrix * n))
#define normal_object_to_world(n) (NormalMatrix * n)
#define normal_world_to_object(n) (NormalMatrixInverse * n)
#define normal_world_to_view(n) (mat3(ViewMatrixGoo) * n)
#define normal_view_to_world(n) (mat3(ViewMatrixInverseGoo) * n)

#define point_object_to_ndc(p) \
  (ProjectionMatrixGoo * (ViewMatrixGoo * vec4((ModelMatrixGoo * vec4(p, 1.0)).xyz, 1.0)))
#define point_object_to_view(p) ((ViewMatrixGoo * vec4((ModelMatrixGoo * vec4(p, 1.0)).xyz, 1.0)).xyz)
#define point_object_to_world(p) ((ModelMatrixGoo * vec4(p, 1.0)).xyz)
#define point_view_to_object(p) ((ModelMatrixInverseGoo * (ViewMatrixInverseGoo * vec4(p, 1.0))).xyz)
#define point_world_to_object(p) ((ModelMatrixInverseGoo * vec4(p, 1.0)).xyz)

vec4 point_view_to_ndc(vec3 p)
{
  return ProjectionMatrixGoo * vec4(p, 1.0);
}

vec3 point_view_to_world(vec3 p)
{
  return (ViewMatrixInverseGoo * vec4(p, 1.0)).xyz;
}

vec4 point_world_to_ndc(vec3 p)
{
  return ProjectionMatrixGoo * (ViewMatrixGoo * vec4(p, 1.0));
}

vec3 point_world_to_view(vec3 p)
{
  return (ViewMatrixGoo * vec4(p, 1.0)).xyz;
}
...
float get_homogenous_z_offset(float vs_z, float hs_w, float vs_offset)
{
  if (vs_offset == 0.0) {
  ...
  else if (ProjectionMatrixGoo[3][3] == 0.0) {
    ...
    return ProjectionMatrixGoo[3][2] * (vs_offset / (vs_z * (vs_z + vs_offset))) * hs_w;
  }
  else {
    return ProjectionMatrixGoo[2][2] * vs_offset * hs_w;
  }
}
...
float get_view_z_from_depth(float depth)
{
  ...
  if (ProjectionMatrixGoo[3][3] == 0.0) {
    d = -ProjectionMatrixGoo[3][2] / (d + ProjectionMatrixGoo[2][2]);
  }
  else {
    d = (d - ProjectionMatrixGoo[3][2]) / ProjectionMatrixGoo[2][2];
  }
  return d;
}
...
float get_depth_from_view_z(float z)
{
  float d;
  if (ProjectionMatrixGoo[3][3] == 0.0) {
    d = (-ProjectionMatrixGoo[3][2] / z) - ProjectionMatrixGoo[2][2];
  }
  else {
    d = ProjectionMatrixGoo[2][2] * z + ProjectionMatrixGoo[3][2];
  }
  return d * 0.5 + 0.5;
}
...
{
  vec4 ndc = ProjectionMatrixGoo * vec4(view, 1.0);
...
vec3 get_view_space_from_depth(vec2 uvcoords, float depth)
{
  ...
  vec4 p = ProjectionMatrixInverseGoo * vec4(ndc, 1.0);
  return p.xyz / p.w;
}

vec3 get_world_space_from_depth(vec2 uvcoords, float depth)
{
  return (ViewMatrixInverseGoo * vec4(get_view_space_from_depth(uvcoords, depth), 1.0)).xyz;
}
...
vec3 get_view_vector_from_screen_uv(vec2 uvcoords)
{
  if (ProjectionMatrixGoo[3][3] == 0.0) {
    ...
    /* This is the manual inversion of the ProjectionMatrixGoo. */
    vec3 vV = vec3((-ndc - ProjectionMatrixGoo[2].xy) /
                       vec2(ProjectionMatrixGoo[0][0], ProjectionMatrixGoo[1][1]),
                   -ProjectionMatrixGoo[2][2] - ProjectionMatrixGoo[3][2]);
    return normalize(vV);
  }
  /* Orthographic case. */
