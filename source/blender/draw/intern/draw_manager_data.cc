...
void drw_resource_buffer_finish(DRWData *data)
{
  int chunk_id = DRW_handle_chunk_get(&drw_get().resource_handle);
  int elem_id = DRW_handle_id_get(&drw_get().resource_handle);
  ...
  if (data->matrices_ubo == nullptr) {
    data->matrices_ubo = static_cast<GPUUniformBuf **>(MEM_callocN(list_size, __func__));
    data->obinfos_ubo = static_cast<GPUUniformBuf **>(MEM_callocN(list_size, __func__));
    data->ubo_len = ubo_len;
  }

  /* Remove unnecessary buffers */
  for (int i = ubo_len; i < data->ubo_len; i++) {
    GPU_uniformbuf_free(data->matrices_ubo[i]);
    GPU_uniformbuf_free(data->obinfos_ubo[i]);
  }

  if (ubo_len != data->ubo_len) {
    data->matrices_ubo = static_cast<GPUUniformBuf **>(
        MEM_recallocN(data->matrices_ubo, list_size));
    data->obinfos_ubo = static_cast<GPUUniformBuf **>(
        MEM_recallocN(data->obinfos_ubo, list_size));
    data->ubo_len = ubo_len;
  }

  /* Create/Update buffers. */
  for (int i = 0; i < ubo_len; i++) {
    void *data_obmat = BLI_memblock_elem_get(data->obmats, i, 0);
    void *data_infos = BLI_memblock_elem_get(data->obinfos, i, 0);
    if (data->matrices_ubo[i] == nullptr) {
      data->matrices_ubo[i] = GPU_uniformbuf_create(sizeof(DRWObjectMatrix) *
                                                        DRW_RESOURCE_CHUNK_LEN);
      data->obinfos_ubo[i] = GPU_uniformbuf_create(sizeof(DRWObjectInfos) *
                                                       DRW_RESOURCE_CHUNK_LEN);
    }
    GPU_uniformbuf_update(data->matrices_ubo[i], data_obmat);
    GPU_uniformbuf_update(data->obinfos_ubo[i], data_infos);
  }

  DRW_uniform_attrs_pool_flush_all(data->obattrs_ubo_pool);
  ...
  BLI_memblock_iternew(data->commands, &iter);
                                          ...
                                          int arraysize)
{
  ...
  if (!unichunk || unichunk->uniform_used == unichunk->uniform_len) {
    unichunk = static_cast<DRWUniformChunk *>(BLI_memblock_alloc(drw_get().data->uniforms));
...
static void drw_call_obinfos_init(DRWObjectInfos *ob_infos, const blender::draw::ObjectRef &ob_ref)
{
  ...
  /* Random float value. */
  uint random = (ob_ref.dupli_object) ?
                    ob_ref.dupli_object->random_id :
  ...
  if (ob->base_flag & BASE_FROM_DUPLI) {
    ob_infos->ob_flag += (ob_ref.dupli_parent == drw_get().draw_ctx.obact) ? (1 << 4) : 0;
  }
  else {
    ob_infos->ob_flag += (ob == drw_get().draw_ctx.obact) ? (1 << 4) : 0;
  }
...
static DRWResourceHandle drw_resource_handle_new(const float (*obmat)[4], const Object *ob)
{
  DRWCullingState *culling = static_cast<DRWCullingState *>(
      BLI_memblock_alloc(drw_get().data->cullstates));
  DRWObjectMatrix *ob_mats = static_cast<DRWObjectMatrix *>(
      BLI_memblock_alloc(drw_get().data->obmats));
  ...
  DRWObjectInfos *ob_infos = static_cast<DRWObjectInfos *>(
      BLI_memblock_alloc(drw_get().data->obinfos));
  UNUSED_VARS(ob_infos);

  DRWResourceHandle handle = drw_get().resource_handle;
  DRW_handle_increment(&drw_get().resource_handle);
...
{
  DRWResourceHandle handle = drw_get().ob_handle;
  if (handle == 0) {
    /* Handle not yet allocated. Return next handle. */
    handle = drw_get().resource_handle;
  }
  return handle & ~(1u << 31);
}
...
static DRWResourceHandle drw_resource_handle(DRWShadingGroup *shgroup,
                                             const float (*obmat)[4],
                                             const blender::draw::ObjectRef &ob_ref)
{
  Object *ob = ob_ref.object;
  ...
  if (drw_get().ob_handle == 0) {
    drw_get().ob_handle = drw_resource_handle_new(obmat, ob);
    drw_get().ob_state_obinfo_init = false;
  }

  if (shgroup->objectinfo) {
    if (!drw_get().ob_state_obinfo_init) {
      drw_get().ob_state_obinfo_init = true;
      DRWObjectInfos *ob_infos = static_cast<DRWObjectInfos *>(
          DRW_memblock_elem_from_handle(drw_get().data->obinfos, &drw_get().ob_handle));

      drw_call_obinfos_init(ob_infos, ob_ref);
    }
  }

  if (shgroup->uniform_attrs) {
    drw_uniform_attrs_pool_update(drw_get().data->obattrs_ubo_pool,
...
static void *drw_command_create(DRWShadingGroup *shgroup, eDRWCommandType type)
{
  ...
  if (chunk == nullptr) {
    DRWCommandSmallChunk *smallchunk = static_cast<DRWCommandSmallChunk *>(
        BLI_memblock_alloc(drw_get().data->commands_small));
    ...
    chunk = static_cast<DRWCommandChunk *>(BLI_memblock_alloc(drw_get().data->commands));
                         ...
                         void *user_data)
{
  ...
  if (G.f & G_FLAG_PICKSEL) {
    drw_command_set_select_id(shgroup, nullptr, drw_get().select_id);
  }
  ...
  if (user_data || bypass_culling) {
    DRWCullingState *culling = static_cast<DRWCullingState *>(
        DRW_memblock_elem_from_handle(drw_get().data->cullstates, &drw_get().ob_handle));
                                           ...
                                           uint tri_count)
{
  ...
  drw_shgroup_call_procedural_add_ex(shgroup, geom, ob_ref, tri_count * 3);
}
...
static void draw_pbvh_nodes(const blender::draw::ObjectRef &ob_ref,
                            ...
                            const blender::IndexMask &nodes_to_draw)
{
  nodes_to_draw.foreach_index([&](const int i) {
     ...
     * object. */
    DRW_shgroup_call_no_cull(shgrp, batches[i], ob_ref);
  });
}
                                       ...
                                       GPUPrimType prim_type)
{
  ...
  DRWCallBuffer *callbuf = static_cast<DRWCallBuffer *>(
      BLI_memblock_alloc(drw_get().data->callbuffers));
  callbuf->buf = DRW_temp_buffer_request(drw_get().data->idatalist, format, &callbuf->count);
  ...
  if (G.f & G_FLAG_PICKSEL) {
    ...
    callbuf->buf_select = DRW_temp_buffer_request(
        drw_get().data->idatalist, &inst_select_format, &callbuf->count);
  ...
  blender::gpu::Batch *batch = DRW_temp_batch_request(
      drw_get().data->idatalist, callbuf->buf, prim_type);
                                                ...
                                                blender::gpu::Batch *geom)
{
  ...
  blender::gpu::Batch *batch = DRW_temp_batch_instance_request(
      drw_get().data->idatalist, callbuf->buf, nullptr, geom);
  drw_command_draw(shgroup, batch, handle);
...
void DRW_buffer_add_entry_array(DRWCallBuffer *callbuf, const void *attr[], uint attr_len)
{
  ...
  if (G.f & G_FLAG_PICKSEL) {
    ...
    GPU_vertbuf_attr_set(callbuf->buf_select, 0, callbuf->count, &drw_get().select_id);
  }
...
static DRWShadingGroup *drw_shgroup_create_ex(GPUShader *shader, DRWPass *pass)
{
  DRWShadingGroup *shgroup = static_cast<DRWShadingGroup *>(
      BLI_memblock_alloc(drw_get().data->shgroups));
                                         ...
                                         GPUSamplerState state)
{
  ...
  GPUTexture **gputex_ref = static_cast<GPUTexture **>(BLI_memblock_alloc(drw_get().data->images));
  *gputex_ref = gputex;
...
GPUVertFormat *DRW_shgroup_instance_format_array(const DRWInstanceAttrFormat attrs[],
                                                 int arraysize)
{
  GPUVertFormat *format = MEM_callocN<GPUVertFormat>(__func__);
...
DRWShadingGroup *DRW_shgroup_create_sub(DRWShadingGroup *shgroup)
{
  ...
  *shgroup_new = *shgroup;
  ...
  DRWPass *parent_pass = static_cast<DRWPass *>(
      DRW_memblock_elem_from_handle(drw_get().data->passes, &shgroup->pass_handle));
...
{
  DRWView *view = static_cast<DRWView *>(BLI_memblock_alloc(drw_get().data->views));

  if (drw_get().primary_view_num < MAX_CULLED_VIEWS) {
    view->culling_mask = 1u << drw_get().primary_view_num++;
  }
  else {
    BLI_assert(0);
                     ...
                     const float (*culling_winmat)[4])
{
  ...
  BLI_assert(view != drw_get().view_default);
  BLI_assert(view->parent == nullptr);
...
{
  return drw_get().view_default;
}

void DRW_view_reset()
{
  drw_get().view_default = nullptr;
  drw_get().view_active = nullptr;
  drw_get().view_previous = nullptr;
}

void DRW_view_default_set(const DRWView *view)
{
  BLI_assert(drw_get().view_default == nullptr);
  drw_get().view_default = (DRWView *)view;
}
...
{
  view = (view) ? view : drw_get().view_default;
...
{
  DRWPass *pass = static_cast<DRWPass *>(BLI_memblock_alloc(drw_get().data->passes));
  ...
  pass->handle = drw_get().pass_handle;
  DRW_handle_increment(&drw_get().pass_handle);
...
{
  const float4x4 &viewinv = drw_get().view_active->storage.viewinv;
  ...
  do {
    ...
    DRWObjectMatrix *obmats = static_cast<DRWObjectMatrix *>(
        DRW_memblock_elem_from_handle(drw_get().data->obmats, &handle));
