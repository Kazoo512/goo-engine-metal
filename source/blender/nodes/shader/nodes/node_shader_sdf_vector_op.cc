...
static void node_shader_update_sdf_vector_op(bNodeTree *ntree, bNode *node)
{
  ...
  /* Out sockets. */
  blender::bke::node_set_socket_availability(*ntree, *sockValueOut,
                            ELEM(sdf->operation,
  ...
  blender::bke::node_set_socket_availability(*ntree, *sockPositionOut,
                            ELEM(sdf->operation, SHD_SDF_VEC_OP_MIRROR, SHD_SDF_VEC_OP_GRID));

  /* In sockets. */
  blender::bke::node_set_socket_availability(*ntree, *sockVector2,
                            ELEM(sdf->operation,
  ...
  blender::bke::node_set_socket_availability(*ntree, *sockVector3,
                            ELEM(sdf->operation,
                                 SHD_SDF_VEC_OP_SWIRL,
                                 SHD_SDF_VEC_OP_RADIAL_SHEAR,
                                 SHD_SDF_VEC_OP_REPEAT_FINITE));
  ...
  /* Values */
  blender::bke::node_set_socket_availability(*ntree, *sockScale,
                            ELEM(sdf->operation, SHD_SDF_VEC_OP_SCALE_UV, SHD_SDF_VEC_OP_TILESET));

  blender::bke::node_set_socket_availability(*ntree, *sockValue,
                            !ELEM(sdf->operation,
  ...
  blender::bke::node_set_socket_availability(*ntree, *sockAngle,
                            ELEM(sdf->operation,
                                 SHD_SDF_VEC_OP_BEND,
                                 SHD_SDF_VEC_OP_TWIST,
                                 SHD_SDF_VEC_OP_ROTATE,
                                 SHD_SDF_VEC_OP_ROTATE_UV));
  blender::bke::node_set_socket_availability(*ntree, *sockCount, ELEM(sdf->operation, SHD_SDF_VEC_OP_TILESET));
  blender::bke::node_set_socket_availability(*ntree, *sockCount2, ELEM(sdf->operation, SHD_SDF_VEC_OP_TILESET));
